...
# This script runs after patch_paybot_1211.py. That first script ports only the
# Minecraft API calls while preserving the original hardcoded messages and
# denomination arrays. Here we add independent Forge and NeoForge targets.

# Build identity and target platforms.
...
# Keep only common + Forge + NeoForge modules.
write(
    "settings.gradle",
    '''pluginManagement {
    repositories {
...
# Root build: Java 21, current Shadow plugin, no references to omitted modules.
p = Path("build.gradle")
s = read(str(p))
...
# Common dependencies are consumed and shaded by both loader JARs. No message
# manager or dynamic denomination layer is introduced here.
write(
    "common/build.gradle",
    '''architectury {
...
configurations {
    ...
    compileClasspath.extendsFrom embeddedLibraries
    runtimeClasspath.extendsFrom embeddedLibraries
}
...
dependencies {
    ...
    embeddedLibraries 'com.google.zxing:core:3.5.3'
    embeddedLibraries 'com.google.zxing:javase:3.5.3'
    embeddedLibraries 'org.nanohttpd:nanohttpd:2.3.1'
    embeddedLibraries('org.xerial:sqlite-jdbc:3.49.1.0') { transitive = false }
    embeddedLibraries('com.mysql:mysql-connector-j:9.2.0') { transitive = false }
    embeddedLibraries 'com.zaxxer:HikariCP:5.1.0'
}
''',
)
...
dependencies {
    ...
    shade(project(path: ":common", configuration: "embeddedLibraries")) { transitive = true }
}
...
public final class ForgeDependencyValidator {
    ...
    public static void validate() {
        ...
        if (!hasMod || !hasClass) {
            throw new IllegalStateException("[PayBot Forge] Missing required Architectury API 13.x dependency.");
        }
    }
}
''',
...
# NeoForge loader. It is independent instead of pretending the Forge artifact is
...
public final class NeoForgeDependencyValidator {
    ...
    public static void validate() {
        ...
        boolean hasClass = DependencyChecker.isClassPresent("dev.architectury.platform.Platform");
        if (!hasMod || !hasClass) {
            throw new IllegalStateException("[PayBot NeoForge] Missing required Architectury API 13.x dependency.");
        }
    }
}
''',
...
# Update fallback build label only. All hardcoded user-facing strings and the
# original fixed denomination arrays remain untouched.
p = Path("common/src/main/java/com/naptien/PayBotMod.java")
s = read(str(p)).replace("5.5.0-mc1.21.1-fullpatch", "5.5.0-mc1.21.1-hardcoded-fix")
s = s.replace("PayBot Fabric v", "PayBot v")
write(str(p), s)

# Assert the request's constraints.
