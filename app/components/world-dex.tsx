...
function expandAddonChunk(chunk:AddonChunk):DexPokemon[]{return chunk.p.map(([key,name,kind,variants])=>({key,name,kind:kind==="f"?"fakemon":"addon-form",sourceLabel:chunk.s,variants:variants.map(([biomes,structures,level,rarity,time,context,presets,notes],i)=>({id:`${key}-${i}`,biomes,biomeLabels:biomes.map(humanizeId),structures,structureLabels:structures.map(humanizeId),level,rarity,rarityLabel:{common:"Phổ biến",uncommon:"Ít gặp",rare:"Hiếm","ultra-rare":"Cực hiếm"}[rarity]||humanizeId(rarity),time,timeLabel:{day:"Ban ngày",night:"Ban đêm",dusk:"Hoàng hôn",dawn:"Bình minh",any:"Mọi lúc"}[time]||humanizeId(time),context,contextLabel:{grounded:"Trên mặt đất",submerged:"Dưới nước",fishing:"Câu cá",natural:"Tự nhiên"}[context]||humanizeId(context),presets,notes:notes?notes.split("|").filter(Boolean):[],source:chunk.s,sourceLabel:chunk.s}))}));}
...
function sourceName(p:DexPokemon,v?:SpawnVariant){return v?.sourceLabel || p.sourceLabel || (p.kind==="pokemon"?"Cobblemon / Server Pack":"Server Addon");}
...
function useDex(){
  ...
  useEffect(()=>{
    const chunkNames=["lively-1","lively-2","lively-3","lively-4","lively-5","fai-1","fai-2","eldoria-1"];
    Promise.all([loadGzipJson(`${BASE_PATH}/data/spawns.json.gz`),...chunkNames.map(name=>fetch(`${BASE_PATH}/data/fakemon/${name}.json`).then(r=>{if(!r.ok)throw new Error("addon dex unavailable");return r.json() as Promise<AddonChunk>;}))]).then(([base,...chunks])=>{
      const official=(base as DexFile).pokemon.map(p=>({...p,kind:"pokemon" as const,sourceLabel:"Cobblemon / Server Pack"}));
      const addons=(chunks as AddonChunk[]).flatMap(expandAddonChunk);
      setRows([...official,...addons]);
      const map=new Map<string,string>(); ((base as DexFile).locations||[]).forEach(x=>map.set(x.id,x.label));
      addons.flatMap(p=>p.variants).forEach(v=>{v.biomes.forEach((id,i)=>map.set(id,v.biomeLabels[i]||humanizeId(id)));v.structures.forEach((id,i)=>map.set(id,v.structureLabels[i]||humanizeId(id)));});
...
function VariantDetail({pokemon}:{pokemon:DexPokemon}){
  return <div className="dex-variant-list">{pokemon.variants.map((v,i)=><article className="dex-variant" key={`${v.source}-${v.id}-${i}`}>
    <div className="dex-variant-title"><div><small>{sourceName(pokemon,v)}</small><strong>{locationLabels(v).join(" · ")}</strong></div><span>{v.rarityLabel}</span></div>
    ...
    {!!v.notes?.length&&<div className="dex-condition-lines">{v.notes.map((n,j)=><span key={j}>{n}</span>)}</div>}
  </article>)}</div>;
}

export default function WorldDex(){
  const {rows,locations,error}=useDex(); const [q,setQ]=useState("");const [kind,setKind]=useState<KindFilter>("all");const [location,setLocation]=useState("all");const [time,setTime]=useState("all");const [rarity,setRarity]=useState("all");const [open,setOpen]=useState<string|null>(null);const [page,setPage]=useState(1);
  const filtered=useMemo(()=>rows.flatMap(p=>{
    ...
    const variants=p.variants.filter(v=>{
      ...
      return normalize([...v.biomeLabels,...v.structureLabels,...v.notes,sourceName(p,v)].join(" ")).includes(needle);
    });
    return variants.length?[{...p,variants}]:[];
  }),[rows,q,kind,location,time,rarity]);
  ...
  const controls=<>
    <label className="dex-search"><span>Tìm tên / biome / condition</span><input value={q} onChange={e=>{setQ(e.target.value);resetPage()}} placeholder="Miraidon, Anglerine, deep ocean…"/></label>
  ...
  <div className="dex-rarity-tabs">{[["all","Mọi độ hiếm"],["common","Phổ biến"],["uncommon","Ít gặp"],["rare","Hiếm"],["ultra-rare","Cực hiếm"]].map(([id,label])=><button key={id} className={rarity===id?"active":""} onClick={()=>{setRarity(id);resetPage()}}>{label}</button>)}</div></>;

  const card=(p:DexPokemon,index:number)=>{const id=`${p.key}-${index}`;const isOpen=open===id;const loc=uniq(p.variants.flatMap(locationLabels));return <article className={`dex-result-card ${isOpen?"open":""}`} key={id}>
    <button className="dex-card-head" type="button" onClick={()=>setOpen(isOpen?null:id)}>
      ...
      <div><small>{p.sourceLabel||"Cobblemon / Server Pack"}</small><h3>{p.name}</h3><p>{p.variants.length} spawn condition · {loc.slice(0,2).join(" · ")}{loc.length>2?` +${loc.length-2}`:""}</p></div>
      <b>{isOpen?"−":"+"}</b>
    </button>
  ...
  return <>
    <div className="portal-desktop desktop-only"><DesktopPortalNav active="dex"/><main className="portal-main dex-main">
      <section className="dex-desktop-head"><div><div className="portal-eyebrow">WORLD DEX · SERVER DATA</div><h1>Biết chính xác.<br/><span>Spawn ở đâu.</span></h1><p>Tra Pokémon, Fakemon và custom form đang dùng trên SVFrame. Mở từng kết quả để xem biome, structure, level, thời gian và condition.</p></div><div className="dex-stat-stack"><div><b>{rows.length||"—"}</b><span>entries</span></div><div><b>{rows.reduce((n,p)=>n+p.variants.length,0)||"—"}</b><span>conditions</span></div><div><b>{counts.fakemon||"—"}</b><span>fakemon</span></div></div></section>
      <section className="dex-desktop-controls"><div className="dex-control-grid">{controls}</div>{filters}</section>
      <section className="dex-results"><div className="dex-result-title"><div><small>KẾT QUẢ</small><h2>{rows.length?`${filtered.length} entries`:"Đang nạp dữ liệu…"}</h2></div>{(q||kind!=="all"||location!=="all"||time!=="all"||rarity!=="all")&&<button onClick={()=>{setQ("");setKind("all");setLocation("all");setTime("all");setRarity("all");resetPage()}}>Xóa bộ lọc</button>}</div>
    ...
    <div className="portal-mobile mobile-only"><MobilePortalHeader title="World Dex"/><main className="mobile-dex-main">
      <section className="mobile-dex-search"><div><small>WORLD DEX</small><h1>{rows.length||"—"} entries</h1></div>{controls}</section>
